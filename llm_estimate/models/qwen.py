...
class QwenModel(BaseModel):
        ...
        for layer_idx in range(self.specs.layers):
            ...
            q_proj = OpProfile(
                ...
                memory_bytes=(batch_size * seq_len * hidden_size + hidden_size * hidden_size + 
                             batch_size * seq_len * hidden_size) * 2,  # bf16
                is_gemm=True,
            ...
            rope_op = OpProfile(
                ...
                flops=6 * batch_size * seq_len * hidden_size,  # RoPE rotation computation
                memory_bytes=batch_size * seq_len * hidden_size * 2 * 2  # Q和K的RoPE应用
            )
    ...
    def estimate_flops_per_token(self) -> float:
        ...
        基于 Qwen 官方实现和 Transformers 架构的更精确公式
        参考文献：
        ...
        # 1.2 RoPE (旋转位置编码) 计算
        # Qwen 使用 RoPE，需要额外的位置编码计算
        rope_flops = num_heads * head_dim * context_length * 4  # 旋转操作的近似FLOPS
        ...
        # Qwen 使用 SwiGLU 激活函数，包含gate、up、down三个投影
        ...
        # SwiGLU激活: gate(x) * swish(up(x))
        # Swish激活函数: x * sigmoid(x)，需要额外计算
        swish_flops = self.specs.intermediate_size * 3  # sigmoid + multiply
        activation_flops = self.specs.intermediate_size + swish_flops  # gate * swish(up)
        ...
        # Qwen 使用 RMSNorm，每层有2个RMSNorm
        # RMSNorm: sqrt(mean(x^2)) 需要 ~3 * hidden_size 操作
    ...
    def get_performance_analysis(self) -> Dict[str, any]:
        ...
        attention_flops_per_layer = (
            ...
            self.specs.attention_heads * self.specs.head_dim * context_length +  # attention computation
        ...
        ffn_flops_per_layer = (
            ...
            self.specs.intermediate_size * 4  # SwiGLU activation
        ) * 2  # multiply-accumulate
        
        # RoPE FLOPS
        rope_flops_per_layer = (
            self.specs.attention_heads * self.specs.head_dim * context_length * 4
        ) * 2
        
        total_attention_flops = layers * attention_flops_per_layer
        total_ffn_flops = layers * ffn_flops_per_layer
        total_rope_flops = layers * rope_flops_per_layer
        
        # 词汇表相关FLOPS (Qwen有更大的词汇表)
        ...
        return {
            "model_info": {
                ...
                "model_features": ["RoPE", "SwiGLU", "RMSNorm", "GQA"]
            },
            "flops_analysis": {
                ...
                "rope_flops_per_token": total_rope_flops,
                "vocab_flops_per_token": vocab_flops,
                "context_dependency": {
                    "quadratic_terms_flops": layers * self.specs.attention_heads * (context_length ** 2),
                    "linear_terms_flops": total_flops - layers * self.specs.attention_heads * (context_length ** 2),
                    "rope_linear_terms_flops": total_rope_flops,
                }
            },
            ...
            "efficiency_metrics": {
                ...
                "context_scaling_factor": context_length / 2048,  # 相对于2K上下文的扩展因子
                "vocab_overhead_percentage": (vocab_flops / total_flops) * 100,
            },
            "qwen_specific_features": {
                "rope_overhead_percentage": (total_rope_flops / total_flops) * 100,
                "large_vocab_impact": {
                    "vocab_size": self.specs.vocab_size,
                    "embedding_memory_gb": (self.specs.vocab_size * hidden_size * 4) / (1024**3),  # fp32
                    "output_projection_flops": vocab_flops,
                },
                "long_context_support": {
                    "max_context_length": context_length,
                    "kv_cache_scaling": "O(L)" if context_length > 4096 else "Standard",
                    "attention_memory_scaling": "O(L²)" if context_length > 8192 else "Manageable",
                }
            }
        } 